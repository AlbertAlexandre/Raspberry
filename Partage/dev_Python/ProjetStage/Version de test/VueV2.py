...
class VueV2(object):
	...
	def CreateWidgets(self):
		...
		self.vF = IntVar()
		self.MacroSelect = StringVar()
		self.LesMacroDispo = os.listdir("/home/pi/.imagej/plugins")
		
		#Formatage du nom des fichiers
		x = 0
		for i in self.LesMacroDispo:
			if ".ijm" not in i:
				del self.LesMacroDispo[x]
				x = x + 1
				print(self.LesMacroDispo)
		x = 0
		for i in self.LesMacroDispo:
			
			self.LesMacroDispo[x] =  self.LesMacroDispo[x].replace("_"," ")
			self.LesMacroDispo[x] =  self.LesMacroDispo[x].replace(".ijm","")
			x = x + 1
		...
		#frame choix de le macro
		
		self.FrameMacro = Frame(self.__root, relief=GROOVE, pady = 3)
		self.FrameMacro.pack()
		self.lb_Macro = Label(self.FrameMacro, text="Macro voulu : ")
		self.lb_Macro.pack() 
		self.cb_Macro = Combobox(self.FrameMacro, textvariable = self.MacroSelect, values = self.LesMacroDispo, state = 'readonly')
		self.cb_Macro.pack()
		...
		self.__btn_Test = Button(self.__frameChoix, text="Tester", command = self.__TestPhoto__)
		self.__btn_Test.pack(side=LEFT)
